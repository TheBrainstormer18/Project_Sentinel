...
import { getModelInsights } from './engine/mlEngine';
...
export function createExpressApp() {
  ...
  // Path normalizer for Netlify Functions (/api/* forwarded to /.netlify/functions/api)
  ...
  const handleDataUpload = (req: Request, res: Response) => {
    try {
      ...
      if (isCommit) {
        ...
        const invalidRowIndices = new Set(validation.errors.map((e) => e.row - 2));
        const validRowsToImport = rawRows
          .filter((_, idx) => !invalidRowIndices.has(idx))
          .map((r) => ({
            project_code: String(r.project_code || r['Project Code'] || `PRJ-${Date.now()}`).trim(),
            project_name: String(r.project_name || r['Project Name'] || 'Untitled Project').trim(),
    ...
    try {
      ...
      res.json({
        ...
        system_status: {
          ...
          validation_service: 'Data Integrity Gate Active',
  ...
  // 8. POST /projects (Manual Project Creation)
  const handleCreateProject = (req: Request, res: Response) => {
    try {
      ...
      if (!project_code || !project_name || !sector) {
        return res.status(400).json({ error: 'project_code, project_name, and sector are required.' });
      }
      ...
      const rawRow = {
        ...
        implementing_agency: implementing_agency || 'Implementing Agency',
      ...
      const val = validateProjectRecords([rawRow]);
      if (val.valid_rows === 0) {
        return res.status(400).json({ error: 'Project validation failed', details: val.errors[0]?.message || 'Invalid row data' });
      }

      db.importRecords([rawRow]);
      const created = Array.from(db.getAllProjects()).find((p) => p.project_code === project_code);
  ...
  app.post('/api/projects', requireAdmin as any, handleCreateProject);
  app.post('/projects', requireAdmin as any, handleCreateProject);

  // 9. POST /predict/:project_id
  ...
  // 11. POST /api/chat and POST /chat
